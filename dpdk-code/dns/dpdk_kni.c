...
static uint16_t g_src_port = UDP_PORT;
static uint16_t g_dest_port = UDP_PORT;

static uint8_t g_dest_mac_addr[RTE_ETHER_ADDR_LEN] = {0xEC, 0xF4, 0xBB, 0x4A, 0xA3, 0xB2};//{ 0x00, 0x0c, 0x29, 0x18, 0xef, 0x9d };
...
static void do_send_udp(struct rte_mempool *mbuf_pool, unsigned char *data, int length) {
	...
	create_eth_ip_udp_pkt(pkt_data, eth_total_len, g_dest_mac_addr, 
		g_src_ip, g_dest_ip, g_src_port, g_dest_port, data, length);
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
		...
		rte_eth_tx_burst(g_dpdkPortId, 0, pkts_burst, (uint16_t)num_rx_recvd);
...
#if ENABLE_KNI
				rte_kni_tx_burst(kni, mbufs, num_recvd);
