...
int vhost_user_msg_handler(int connfd, struct vhost_user_msg *msg) {
	...
	case VHOST_USER_SET_VRING_ADDR: { // msg --> 
		...
		printf("index: %d\n", msg->state.index);
		printf("gva desc: %lx, avail: %lx, used: %lx, log: %lx\n",
			msg->addr.desc, msg->addr.avail, msg->addr.used, msg->addr.log);
		printf("hva desc: %p, avail: %p, used: %p, desc->addr: %lx\n", 
			vq->desc, vq->avail, vq->used, vq->desc->addr);
		
		break;
	}
	case VHOST_USER_SET_VRING_BASE: {
		printf("set vring base : %d\n", msg->state.num);
...
int copy_desc_to_mbuf(struct virtio_dev *dev, struct virtqueue *vq,
	...
	uint32_t hdrlen = sizeof(struct virtio_net_hdr);

	void *desc_addr = (void*)gpa_to_hva(dev, desc->addr);
	uint32_t desc_pkt_len = desc->len - hdrlen;

	memcpy(&mbuf->hdr, desc_addr, hdrlen);
	if (desc_pkt_len > 0) {
		mbuf->len = desc_pkt_len;
		memcpy(mbuf->data, ((unsigned char *)desc_addr) + hdrlen, desc_pkt_len);
...
int vhost_tx(struct virtio_dev *dev, struct mbuf *pkts[], uint16_t npkts) {
	...
	for (i = 0;i < npkts;i ++) {
		...
		copy_desc_to_mbuf(dev, vq, pkts[i]);
	}
#endif
